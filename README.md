...
1. The first 3 columns and 1st row have been frozen for better navigation. 

2. Columns I and J dates formatting:
    * These columns represent dates. However, they are not in a readable format. To make sure that the numbers in this column are, indeed, timestamps, [**timestamp converter tool**](https://www.epochconverter.com/) was used. 
 ...
 4. The following other columns needed for the analysis were created: 
    ...
    _Percentage Funded_
      ...
      * Then, value shading was applied to this column using "Color Scales" followed by "More Rules" in the Conditional Formatting. 
         * The color "Green, Accent 6, Lighter 80%" is assigned as the minimum color.
         * The color "Green, Accent 6, Darker 25%" is assigned as the maximum one. 

    
    _Average Donation_

      Kickstarter crowdfunding platform provides incentives for different pledge amounts. If you are interested in setting up any incentives when launching a campaign, it might useful to know how much people have pledged historically. 
    ...
    _Campaign Parent Category and Subcategory_
      ...
      * Column N (category_and_subcategory) shows the category and subcategory for each campaign. This column was split using "Text to Columns" in the Data tab ("Delimited" separation and "/" as the delimiter). 
...
The "Category Statistics" sheet contains analysis displaying outcomes for each fundraising category, which can be filtered by country. The chart below shows that, **the theater category was the most successful** with 839 Kickstarter campaigns among all 21 countries.  
...
The "Subcategory Statistics" sheet contains analysis displaying outcomes for each subcategory, which can be filtered by the parent category and country. After determining that the theater category was the most successful, we can see exactly what type of theater campaigns brought the most success. 

The chart shows that **plays were the most successful subcategory of the theater category** with 694 successful campaigns out of 1393 total. 
...
The data below, more or less, exhibits an overall trend. There is a general increase in the number of successful theater campaigns between March and May, where May has the highest number of successful campaigns. After May, there is a general decrease in successful outcomes, reaching the lowest number in December. The number of canceled campaigns stays roughly stable throughout the year with overall low counts, with a maximum of 7 campaigns in January and a minimum of 1 in July. 
...
**The "Outcomes Based on Goals_Plays" sheet** demonstrates the analysis of outcomes for plays based on goal amount ranges. 

The chart below illustrates that the highest percentage of successful plays of 76% had fundraising goals of less than $1000. Overall, with higher goal ranges, there is a general decrease in the successful outcomes and a general increase in the failed outcomes. However, there is an unusual reverse trend observed in the goal ranges between $30,000 to $44,999, where the success rates spiked and failed outcomes slumped. 
...
* The dataset came with non-readable date columns. The conversion is explained [above](https://github.com/Aigerim-Zh/kickstarter-analysis#preparing-the-data). 
* There are many positive outliers in the data, which need to be reviewed. It is possible that, because of these outliers, the trend lines are slightly distorted. 
* It is assumed that the monetary columns such as goal and pledged amounts are standardized to one currency and adjusted for inflation. However, it is not clear from the dataset. 
...
### **Conclusions about Outcomes based on Fundraising Goals of Theater Plays**
  * Plays were the most successful subcategory of the theater category campaigns. 
  * Theater plays that had fundraising goals of less than $1000 resulted in the highest percentage of successful outcomes. 
...
* There is not enough data for "live" category campaigns, which had to be excluded for some parts of the analyses. 
...
Let's look at the following five plays that happened at the Edinburgh Festival Fringe: _Be Prepared, Checkpoint 22, Cutting Off Kate Bush, Jestia and Raedon, and The Hitchhiker's Guide to the Family_. The analysis is located in the "Edinburgh Plays Research" sheet with the data pulled from the "Formatted Data" sheet using the VLOOKUP function. 
...
The "Successful US Plays" and "Failed US Plays" sheets are datasets filtered for successful and failed theater plays launched in the United States. 

The table below shows descriptive statistics based on these data. 
![](https://github.com/Aigerim-Zh/kickstarter-analysis/blob/main/resources/Outcome_vs_Goal_Pledged_US_stats.png)

* The distributions are similar in all subsets with the mean around the 75th percentile (or 3rd quartile). 
...
* In each distribution, the standard deviation is larger than the mean. Any observation below the mean can be considered "close" to the center. 
* **Failed** plays had **higher fundraising goals**, as per the average and median. At the same time, both the mean and median pledged amounts are significantly lower for the failed plays than that for the successful ones. If the **median pledged** amounts were more or less the same for both successful and failed campaigns, we could say that the failed ones asked for too much money. However, it is not the case here - there must be other factors in play. 

## Analysis of Theater Campaigns for Musicals in Great Britain

The below chart displays distributions of goal and pledged amounts for the "Musicals" subcategory in Great Britain. 
...
* The range of outliers for the amount pledged is any value above $3,697. If you are interested in launching musicals in Great Britain, it would probably not be advised to set a fundraising goal above this threshold amount.  
